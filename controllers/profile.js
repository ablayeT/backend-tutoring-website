...
exports.getAllStudentProfile = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: 'Erreur lors de la récuperation des profiles des étudiants',
      });
  }
};
...
exports.createStudentProfile = async (req, res, next) => {
  ...
  try {
    ...
    const newStudent = await knex('profiles').insert({
      user_id,
      photo,
      field_of_study,
      year_of_study,
    ...
    res
      .status(201)
      .json({ message: 'Profil étudiant créé avec succés', id: newStudent[0] });
  } catch (error) {
    res
      .status(500)
      .json({ error: 'Erreur lors de la création du profil étudiant' });
  }
  next();
};
...
exports.createTutorProfile = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) {
      ...
      res
        .status(201)
        .json({ message: 'Profil tuteur créé avec succés', id: newTutor[0] });
    }
  } catch (error) {
    res
      .status(500)
      .json({ error: 'Erreur lors de la création du profil tuteur' });
  }
};
