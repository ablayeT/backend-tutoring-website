...
exports.createStudentProfiles = async (req, res, next) => {
  ...
  try {
    ...
    if(!user) {
      ...
      const existingProfile = await knex('student_profiles').where('user_id', id).first();
      if(existingProfile) {
        return res.status(400).json({error: 'Le profil de cet utilisateur existe déjà'}); 
      }
    ...
    return res.status(500).json({error: 'Une erreur est survenue lors de la création du profil de l"étuddiant'})
  }
}
...
exports.rateCommentTutor = async (req, res, next) => {
    ...
    try {
      ...
      if (!student || student.user_type !== 'student') {
        return res.status(400).json({error: 'Vous devez être un étudiant pour laisser une évaluaition'})
      }
      ...
      return res.json({message: 'Evaluation et commentaire ajouté avec succès'})
    }catch (error) {
      return res.status(500).json({error: "Erreur lors de la soumission de l'évaluation et commentaire"});
...
exports.bookSession = async (req, res, next) => {
    ...
    try{
        ...
        const tutor = await knex('users').where({id: tutorId, user_type: 'tutor'}).first(); 
        if(!tutor) {
            return res.status(400).json({error : 'Tutor non existant'}); 
        }

        // Vérifier si l'étudiant existe dans la base de donnée
        const student = await knex('users').where({id: studentId, user_type: 'student'}).first(); 
        if(!student) {
        ...
        return res.json({message : 'Session de turorat resservée avec succès', session_id: sessionId[0]}); 
    }catch (error) {
        return res.status(500).json({error : "Erreur lors de la réservaiton de la session de tutorat"}); 
    }
}
...
exports.getStudentSessions = async (req, res, next) =>  {
    ...
    try {
        ...
        const student = await knex('users').where({id: 'id', user_type: 'student'}).first(); 
        if(!student) {
            return res.status(400).json({error: 'Etudiant non existant'}); 
        }
        ...
        .where('id', id)
        .select('id', 'tutor_id', 'subject_id', 'date', 'start_time', 'end_time', 'location','price', 'status');
        
        return res.json({sessions}); 

    }catch(error) {
        return res.status(400).json({error: "Erreur lors de la recuperation des sessions de tutorat de l'étudiant"});
