...
exports.getOneProfile = async (req, res, next) => {
  ...
  try {
    ...
    res.status(500).json({message : 'Error lors de la recuraptiion du profile.'})
  }
  next()
}
...
exports.deleteProfile  = async (req, res, next) => {
  ...
  try{
    ...
    return res.status(500).json({error: 'Une erreur est survenue lors de la suppression du profil'}); 
  }
}
...
exports.getProfiles = async (req, res, next) =>{
  ...
  try {
    ...
    .leftJoin('student_profiles', 'users.id', '=', 'student_profiles.user_id')
      ...
      .select('users.id', 'users.full_name', 'users.email', 'users.user_type', 'student_profiles.imageUrl as student_imageUrl', 'student_profiles.skills as student_skills', 'student_profiles.experience as student_experience', 'student_profiles.grade_level as student_grade_level', 'student_profiles.major as student_major', 'student_profiles.university as student_university', 'tutor_profiles.imageUrl as tutor_imageUrl', 'tutor_profiles.skills as tutor_skills', 'tutor_profiles.experience as tutor_experience', 'tutor_profiles.hourly_rate', 'tutor_profiles.availability')
      .groupBy('users.id');

      return res.json({profiles});

  }catch(error) {
    return res.status(500).json({error: "Une erreur est survenue lors de la recuperation des profils"})
  }
...
exports.createProfile = async (req, res, next)=> {
  ...
  try {
    ...
    const existingProfile = userType === 'student'
    ? await knex('student_profiles').where('user_id', id).first()
    : await knex('tutor_profiles').where('user_id', id).first()

    if(existingProfile) {
      return res.status(400).json({error: 'Le profile de cet utilisateur existe déjà'}); 
    }
    ...
    return res.status(500).json({error: 'Une erreur est survenue lors de la creation du profil'}); 
  }
}
