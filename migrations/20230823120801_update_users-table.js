...
exports.up = function (knex) {
    return knex.schema.table('users', (table) => {
      ...
      return knex('users').update({
        first_name: knex.raw("CASE WHEN INSTR(full_name, ' ') > 0 THEN SUBSTRING_INDEX(full_name, ' ', 1) ELSE full_name END"),
        last_name: knex.raw("CASE WHEN INSTR(full_name, ' ') > 0 THEN SUBSTRING_INDEX(full_name, ' ', -1) ELSE '' END")
      });
    }).then(() => {
